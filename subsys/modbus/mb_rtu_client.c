...
static int mbm_validate_response_fc(struct mb_rtu_context *pch,
				    const uint8_t node_addr,
				    uint8_t fc)
{
	uint8_t resp_fc = pch->rx_frame.fc;
	uint8_t excep_code = pch->rx_frame.data[0];
	const uint8_t excep_bit = BIT(7);
	const uint8_t excep_mask = BIT_MASK(7);

	if (node_addr != pch->rx_frame.addr) {
		return -EIO;
	}
...
static int mbm_validate_fc03fp_response(struct mb_rtu_context *pch, float *ptbl)
{
	...
	resp_byte_cnt = pch->rx_frame.data[0];
	resp_data = &pch->rx_frame.data[1];
	req_qty = sys_get_be16(&pch->tx_frame.data[2]);
...
static int mbm_validate_rd_response(struct mb_rtu_context *pch,
				    const uint8_t node_addr,
				    uint8_t fc,
				    uint8_t *data)
{
	...
	resp_data = &pch->rx_frame.data[1];
	req_qty = sys_get_be16(&pch->tx_frame.data[2]);
	req_addr = sys_get_be16(&pch->tx_frame.data[0]);
	...
	case MODBUS_FC03_HOLDING_REG_RD:
		if (IS_ENABLED(CONFIG_MODBUS_RTU_FP_EXTENSIONS) &&
			...
			err = mbm_validate_fc03fp_response(pch, (float *)data);
			break;
		}
		/* fallthrough */
...
static int mbm_validate_fc08_response(struct mb_rtu_context *pch,
				      const uint8_t node_addr,
				      uint16_t *data)
{
	int err;
	...
	req_sfunc = sys_get_be16(&pch->tx_frame.data[0]);
	req_data = sys_get_be16(&pch->tx_frame.data[2]);
	resp_sfunc = sys_get_be16(&pch->rx_frame.data[0]);
	resp_data = sys_get_be16(&pch->rx_frame.data[2]);
...
static int mbm_validate_wr_response(struct mb_rtu_context *pch,
				    const uint8_t node_addr,
				    uint8_t fc)
{
	int err;
	...
	req_addr = sys_get_be16(&pch->tx_frame.data[0]);
	req_value = sys_get_be16(&pch->tx_frame.data[2]);
	resp_addr = sys_get_be16(&pch->rx_frame.data[0]);
	resp_value = sys_get_be16(&pch->rx_frame.data[2]);
			...
			uint8_t fc, void *data)
{
	...
	case MODBUS_FC07_EXCEPTION_STATUS_RD:
		*(uint8_t *)data = ctx->rx_frame.data[0];
		break;
		      ...
		      const uint16_t num_coils)
{
	...
	err = mbm_send_cmd(ctx, node_addr, 1, coil_tbl);
			...
			const uint16_t num_di)
{
	...
	err = mbm_send_cmd(ctx, node_addr, 2, di_tbl);
	k_mutex_unlock(&ctx->iface_lock);
			     ...
			     const uint16_t num_regs)
{
	...
	err = mbm_send_cmd(ctx, node_addr, 3, reg_buf);
	k_mutex_unlock(&ctx->iface_lock);
		      ...
		      const bool coil_state)
{
	...
	err = mbm_send_cmd(ctx, node_addr, 5, NULL);
	k_mutex_unlock(&ctx->iface_lock);
			     ...
			     const uint16_t reg_val)
{
	...
	err = mbm_send_cmd(ctx, node_addr, 6, NULL);
	k_mutex_unlock(&ctx->iface_lock);
				 ...
				 uint8_t *const status)
{
	...
	ctx->tx_frame.length = 0;
	err = mbm_send_cmd(ctx, node_addr,
			      ...
			      uint16_t *const data_out)
{
	...
	err = mbm_send_cmd(ctx, node_addr, 8, data_out);
	k_mutex_unlock(&ctx->iface_lock);
		       ...
		       const uint16_t num_coils)
{
	...
	p_data = &ctx->tx_frame.data[5];
	ctx->tx_frame.length = 5 + num_bytes;

	memcpy(p_data, coil_tbl, num_bytes);

	err = mbm_send_cmd(ctx, node_addr, 15, NULL);
	k_mutex_unlock(&ctx->iface_lock);
			      ...
			      const uint16_t num_regs)
{
	...
	for (int i = 0; i < num_regs; i++) {
		sys_put_be16(reg_buf[i], p_data);
		p_data += sizeof(uint16_t);
	}

	err = mbm_send_cmd(ctx, node_addr, 16, NULL);
	k_mutex_unlock(&ctx->iface_lock);
